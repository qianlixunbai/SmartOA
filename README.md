...
│   ├── service/             # ビジネスロジック層（5 ファイル）
│   ├── repository/          # データアクセス層（6 ファイル）
│   ├── entity/              # エンティティクラス（6 ファイル）
│   └── dto/                 # データ転送オブジェクト
├── src/main/resources/
│   └── application.properties
...
│       ├── stores/          # Pinia 状態管理
│       ├── router/          # ルーティング設定
│       ├── views/           # ページコンポーネント（7 ページ）
│       ├── components/      # 共有コンポーネント
│       └── layouts/         # レイアウトコンポーネント
├── docs/
│   ├── mysql-p0-upgrade.sql # テーブル作成 + シードデータ
│   └── mysql-p1-upgrade.sql # P1 追加テーブル + シードデータ
└── pom.xml
```
...
- [x] 6 テーブル（+ `approval_node`、`template_field`）
- [x] 設定可能な多段階承認エンジン（動的ノード巡回、任意階層対応）
- [x] 承認 / 却下 / 取り消し / 転送の 4 アクション
- [x] 承認ノード設定 UI（テンプレート編集時に追加・削除・並べ替え可能）
- [x] フロー進捗バー（`el-steps`、動的ノード）
- [x] 承認履歴タイムライン（`el-timeline`、操作タイプを色分け）
- [x] ECharts 統計チャート（テンプレート別平均承認時間 + 使用量分布）
...
- [ ] フロントエンド Excel エクスポートボタン（バックエンド API は完了）
- [ ] 動的フォームレンダリング（`template_field` に基づく自動生成）
- [ ] `template_field` バックエンド CRUD API
- [ ] ビジュアルフローエディタ（ノードのドラッグ＆ドロップ + 連線）
- [ ] 承認条件分岐（例：金額 > 10000 で自動的に財務承認追加）
- [ ] 並行承認ノード + CC 機能
- [ ] タイムアウト自動エスカレーション（`@Scheduled`）
